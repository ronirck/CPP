...
// * Se define una clase plantilla "Queue" que puede almacenar nodos de cualquier tipo.
template <class elem>
class Queue
{
private:
    Node<elem> *prim; // * Puntero al primer nodo de la cola.
    Node<elem> *ult;  // * Puntero al último nodo de la cola.

public:
    // ? Constructor
    Queue() : prim(nullptr), ult(nullptr) {} // * Inicializa el puntero prim y ult a nullptr, indicando que la cola está vacía.
    ...
    ~Queue() // * Destructor que se llama al destruir un objeto de la clase Queue.
    {
...
int main()
{
    Queue<int> queue; // * Crea una cola de enteros.
...
// - y una clase plantilla "Queue" que representa la cola enlazada en sí misma.
// - La clase "Node" incluye métodos para obtener y establecer el dato y el puntero al siguiente nodo.
// - La clase "Queue" incluye métodos para agregar nodos a la cola (enColar), eliminar nodos de la cola (desEnColar),
